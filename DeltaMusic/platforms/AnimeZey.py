...
from DeltaMusic.utils import get_readable_time, convert_bytes
...
class AnimeZey:
    ...
    **Pengunduh AnimeZey 📥**
    ...
    💾 **Ukuran Total Berkas:** {total_size}
    ✅ **Selesai:** {completed_size} 
    📊 **Progres:** {percentage[:5]}%
    ...
    ⏳ **Waktu Tersisa:** {eta}"""
            ...
            try:
                async with session.get(f"{self.base_url}{link}") as response:
                    ...
                    await mystic.edit_text(
                        "✅ Unduhan selesai...\n📂 Memproses berkas sekarang"
                    )
                    downloader.pop("eta", None)
                    downloader.pop(download_id, None)
                    return True
            except Exception as e:
                await mystic.edit_text(f"Gagal mengunduh: {str(e)}")
                return False
        ...
        if len(downloader) > 10:
            ...
            try:
                ...
                eta = "Tidak diketahui"
            await mystic.edit_text(
                f"Terlalu banyak unduhan bersamaan! Perkiraan waktu tunggu: {eta}."
            )
            return False
...
async def stop_downloading_animezey(_, callback_query: CallbackQuery):
    ...
    await callback_query.edit_message_text("🚦 Unduhan dibatalkan.")