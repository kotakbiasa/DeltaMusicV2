...
async def atag_all_useres(_, message):
    ...
    if message.chat.id in SPAM_CHATS:
        return await message.reply_text(
            "Proses penandaan sudah berjalan. Gunakan /acancel untuk menghentikan."
        )
    replied = message.reply_to_message
    if len(message.command) < 2 and not replied:
        await message.reply_text(
            "**Berikan teks untuk menandai semua, seperti »** `@aall Hi Friends`"
        )
        return
    if replied:
...
async def cancelcmd(_, message):
    ...
    if chat_id in SPAM_CHATS:
        try:
            ...
            pass
        return await message.reply_text("**Proses penandaan berhasil dihentikan!**")

    else:
        await message.reply_text("**Tidak ada proses yang sedang berjalan!**")
        return
