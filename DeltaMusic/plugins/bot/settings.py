...
from DeltaMusic.utils.decorators.admins import ActualAdminCB
from DeltaMusic.utils.decorators.language import language, languageCB
from DeltaMusic.utils.inline.settings import (
    auth_users_markup,
    playmode_users_markup,
    setting_markup,
    vote_mode_markup,
)
from DeltaMusic.utils.inline.start import private_panel
...
async def gib_repo(client, CallbackQuery, _):
    await CallbackQuery.edit_message_media(
        ...
        reply_markup=InlineKeyboardMarkup(
            [[InlineKeyboardButton(text="Kembali", callback_data=f"settingsback_helper")]]
        ),
    )
...
async def settings_cb(client, CallbackQuery, _):
    ...
    if CallbackQuery.data == "back":
        ...
        await CallbackQuery.message.edit_text(
            text=_["back_message"],  # Replace with the actual message for the "back" button
            reply_markup=back_markup  # Replace with the actual markup for the "back" button
        )
        return
