...
from DeltaMusic import Carbon, app
from DeltaMusic.utils.decorators.language import language, languageCB
from DeltaMusic.utils.inline.playlist import (
    botplaylist_markup,
    get_playlist_markup,
    warning_markup,
)
    ...
    if current_time - last_message_time < SPAM_WINDOW_SECONDS:
        ...
        if user_command_count[user_id] > SPAM_THRESHOLD:
            ...
            hu = await message.reply_text(
                f"**{message.from_user.mention} Tolong jangan spam, coba lagi setelah 5 detik**"
            )
...
async def add_playlist(client, message: Message, _):
    if len(message.command) < 2:
        return await message.reply_text(
            "**Silakan berikan saya nama lagu atau tautan lagu atau tautan playlist YouTube setelah perintah..**\n\n**Contoh:**\n\n▷ `/addplaylist Blue Eyes` (Masukkan nama lagu tertentu)\n\n▷ /addplaylist [tautan playlist YouTube] (Untuk menambahkan semua lagu dari playlist YouTube ke playlist bot.)"
        )
    ...
    if "youtube.com/playlist" in query:
        adding = await message.reply_text(
            "**Menambahkan lagu ke playlist, harap tunggu..**"
        )
        try:
        ...
        if not video_urls:
            return await message.reply_text(
                "**Tidak ada lagu yang ditemukan di tautan playlist.\n\n**Coba tautan playlist lain**"
            )
        ...
        for video_url in video_urls:
            ...
            try:
                ...
                return await message.reply_text(f"Error mengambil info video: {e}")
        ...
        await adding.delete()
        return await message.reply_text(
            text="**Semua lagu telah berhasil ditambahkan dari tautan playlist YouTube Anda✅**\n\n**Jika Anda ingin menghapus lagu, klik tombol di bawah ini.\n\n**Cek dengan » /playlist**\n\n▷ **Putar dengan » /play**",
    ...
    if "youtube.com/@" in query:
        ...
        if not video_urls:
            return await message.reply_text(
                "**Tidak ada lagu yang ditemukan di channel YouTube.\n\n**Coba tautan channel YouTube lain**"
            )
        ...
        await addin.delete()
        return await message.reply_text(
            text="**Semua lagu telah berhasil ditambahkan dari tautan channel YouTube Anda✅**\n\n**Jika Anda ingin menghapus lagu, klik tombol di bawah ini.\n\n**Cek dengan » /playlist**\n\n▷ **Putar dengan » /play**",
            reply_markup=keyboardes,
        )
    ...
    if "https://youtu.be" in query:
        try:
            add = await message.reply_text(
                "**M9enambahkan lagu ke playlist, harap tunggu..**"
            )
            from pytube import Playlist, YouTube
            ...
            try:
                ...
                await message.reply_photo(
                    thumbnail,
                    caption="**Lagu telah ditambahkan ke playlist bot Anda✅**\n\n**Cek dengan » /playlist**\n\n**Hapus dengan » /delplaylist**\n\n**Dan putar dengan » /play (hanya grup)**",
        ...
        from DeltaMusic import YouTube
        ...
        try:
            ...
            m = await message.reply("**Menambahkan, harap tunggu... **")
            ...
            return await message.reply_text("Data tidak valid diterima.")
        except Exception as e:
            pass
...
async def del_plist(client, CallbackQuery, _):
    ...
    keyboards = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "Pulihkan lagu Anda", callback_data=f"recover_playlist {videoid}"
                )
            ]
        ]
    )
    return await CallbackQuery.edit_message_text(
        text="**Lagu Anda telah dihapus dari playlist bot Anda**\n\n**Jika Anda ingin memulihkan lagu Anda di playlist Anda, klik tombol di bawah ini**",
    ...
    try:
        ...
        keyboardss = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        "Hapus lagi", callback_data=f"remove_playlist {videoid}"
                    )
                ]
            ]
        )
        return await CallbackQuery.edit_message_text(
            text="**Lagu dipulihkan di playlist Anda**\n\n**Cek Playlist dengan /playlist**\n\n**Hapus Playlist dengan » /delplaylist**\n\n**Dan putar playlist dengan » /play**",
...
async def add_playlist(client, CallbackQuery, _):
    await CallbackQuery.answer(
        "➻ Untuk menambahkan lagu ke playlist Anda, ketik /addplaylist (Nama lagu Anda)\n\n➥ Contoh » /addplaylist Blue Eyes Blue tyes.",
        show_alert=True,
    )
...
async def del_whole_playlist(client, CallbackQuery, _):
    ...
    for x in _playlist:
        await CallbackQuery.answer(
            "➻ Oke, harap tunggu.\n\n➥ Menghapus playlist Anda...", show_alert=True
        )
