...
from DeltaMusic import app, Platform, LOGGER
...
@app.on_message(
    filters.command(["movies", "movie" , "filmes" , "filme"])
    & filters.group
    & ~BANNED_USERS
)
async def scan_movie_folder(_, message: Message):
    ...
    if not query:
        return await message.reply_text("🎬 Masukkan judul film yang ingin Anda cari.")
