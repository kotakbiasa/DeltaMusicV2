# * @penulis       Yasir Aris M <yasiramunandar@gmail.com>
# * @tanggal       2023-06-21 22:12:27
# * @namaProyek    MissKatyPyro
# * Hak Cipta ©YasirPedia Semua hak dilindungi
import asyncio
import html
...
__MODULE__ = "ChatBot"
__HELP__ = """
/ai - Menghasilkan respons teks dari AI menggunakan Gemini AI oleh Google.
/ask - Menghasilkan respons teks dari AI menggunakan OpenAI.
"""

gptai_pembicaraan = TTLCache(maxsize=4000, ttl=24*60*60)
gemini_pembicaraan = TTLCache(maxsize=4000, ttl=24*60*60)
    ...
    try:
        ...
        if not is_stream:
            ...
            if len(jawaban) > 4000:
                ...
                await bmsg.edit_msg(f"{html.escape(jawaban)}\n<b>Ditenagai oleh:</b> <code>Gemini 1.5 Flash</code>")
        else:
            async for chunk in response:
                if not chunk.choices or not chunk.choices[0].delta.content:
                    continue
                num += 1
                jawaban += chunk.choices[0].delta.content
                if num == 30 and len(jawaban) < 4000:
                    await bmsg.edit_msg(html.escape(jawaban))
        ...
        await bmsg.edit_msg(f"Server tidak dapat dijangkau karena {e.__cause__}")
        return None
    except RateLimitError as e:
        if "billing details" in str(e):
            return await bmsg.edit_msg(
                "Kunci openai dari bot ini telah kedaluwarsa, silakan berikan donasi kunci openai untuk pemilik bot."
            )
        await bmsg.edit_msg("Anda terkena batasan, silakan coba lagi nanti.")
        return None
    except APIStatusError as e:
        await bmsg.edit_msg(
            f"Kode status {e.status_code} lainnya diterima dengan respons {e.response}"
        )
        return None
...
async def gemini_chatbot(_, ctx: Message, strings):
    ...
    if not GOOGLEAI_KEY:
        return await ctx.reply_msg("GOOGLEAI_KEY env tidak ada!!!")
