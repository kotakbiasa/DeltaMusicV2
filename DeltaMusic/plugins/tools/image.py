...
async def google_img_search(client: Client, message: Message):
    ...
    try:
        ...
        return await message.reply("Berikan query gambar untuk mencari!")
    ...
    try:
        ...
        if not os.listdir(images_dir):
            raise Exception("Tidak ada gambar yang diunduh.")
        ...
        return await message.reply(f"Kesalahan dalam mengunduh gambar: {e}")

    msg = await message.reply("Mengunduh gambar...")

    count = 0
    for img in lst:
        count += 1
        await msg.edit(f"=> Mengunduh gambar {count}")
    ...
    try:
        ...
        await msg.delete()
        return await message.reply(f"Kesalahan dalam mengirim gambar: {e}")
