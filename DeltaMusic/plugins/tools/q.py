...
async def msg_quotly_cmd(self: app, ctx: Message):
    ww = await ctx.reply_text("Tunggu sebentar...")
    is_reply = False
    ...
    if len(ctx.text.split()) > 1:
        ...
        if check_arg[0]:
            if check_arg[1] < 2 or check_arg[1] > 10:
                await ww.delete()
                return await ctx.reply_msg("Rentang tidak valid", del_in=6)
            try:
                messages = [
                    i
...
__HELP__ = """
**Perintah Bot Pembuatan Kutipan**

Gunakan perintah ini untuk membuat kutipan dari pesan:

- `/q`: Buat kutipan dari satu pesan.
- `/r`: Buat kutipan dari satu pesan dan pesan yang dibalas.

**Contoh:**
- `/q`: Buat kutipan dari pesan yang dibalas.

- `/r`: Buat kutipan dari pesan yang dibalas.

**Catatan:**
Pastikan untuk membalas pesan agar perintah kutipan berfungsi.
"""
