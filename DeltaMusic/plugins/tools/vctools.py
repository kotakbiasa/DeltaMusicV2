...
async def brah(_, msg):
    await msg.reply("**Video chat dimulai!**")


# vc off
@app.on_message(filters.video_chat_ended)
async def brah2(_, msg):
    await msg.reply("**Video chat berakhir!**")


# invite members on vc
@app.on_message(filters.video_chat_members_invited)
async def brah3(app: app, message: Message):
    text = f"➻ {message.from_user.mention}\n\n**Mengundang ke video chat:**\n\n**➻ **"
    x = 0
    ...
    try:
        ...
        await message.reply(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [InlineKeyboardButton(text="Join Video Chat", url=add_link)],
                ]
            ),
        )
...
def calculate_math(client, message):
    ...
    try:
        ...
        response = "Ekspresi tidak valid"
    message.reply(response)


@app.on_message(filters.command(["spg"], ["/", "!", "."]))
async def search(event):
    msg = await event.respond("Mencari...")
    async with aiohttp.ClientSession() as session:
        ...
        async with session.get(
            ...
            for item in response["items"]:
                ...
                result += f"{title}\n{link}\n\n"
            prev_and_next_btns = [
                Button.inline("▶️Berikutnya▶️", data=f"next {start+10} {event.text.split()[1]}")
            ]
